<?php
    ...
    define("TABLE2","loveActivity",false);
    ...
    {
        $sql="select count(*) from ".TABLE2." where yid=$yid;";
        $result=$db->query($sql);
        if(!$result || $result->num_rows <=0)
        {
            $final["status"]=ERROR;
        ...
        $row=$result->fetch_assoc();
        $presentNum=$row["count(*)"];
        $sql="select maxNum from ".TABLE3." where yid=$yid;";
        $result=$db->query($sql);
        if(!$result || $result->num_rows <=0)
        {
            $final["status"]=ERROR;
        ...
        $row=$result->fetch_assoc();
        $maxNum=$row["maxNum"];
        if(intval($presentNum)>=intval($maxNum))
        {
            $final["status"]=OVERFLOW;
            $db->close();
            echo json_encode($final);
            return;
        }
        $loveTime=date("Y-m-d h:i:s");
        $sql="insert into ".TABLE2." (uid,yid,loveTime) value($uid,$yid,\"$loveTime\");";
        if(!$db->query($sql))
        {
            $final["status"]=ERROR;
