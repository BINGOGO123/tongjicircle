<?php
    ...
    define("TABLE2","commentActivity",false);
    ...
    $yid=dataDeal($_POST["yid"]);
    // $content=$_POST["content"];
    $content=dataDeal($_POST["content"]);
    $commentTime=date("Y-m-d h:i:s");
    $sql="select max(cid) from ".TABLE2." where yid=$yid;";
    $result=$db->query($sql);
    if(!$result || $result->num_rows<=0)
    {
        $cid=0;
    ...
    $sql="insert into ".TABLE2." (uid,yid,cid,cidTarget,content,commentTime) value($uid,$yid,$cid,-1,\"$content\",\"$commentTime\");";
    // echo $sql;
    if(!$db->query($sql))
    {
        $final["status"]=ERROR;
